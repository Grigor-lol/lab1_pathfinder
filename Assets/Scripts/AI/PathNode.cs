...
public class PathNode : IComparable<PathNode>//: MonoBehaviour
{
    ...
    public PathNode(Vector2Int _gridVec, GameObject _objPrefab, bool _walkable, Vector3 position)
    {
        gridVec = _gridVec;
    ...
    {
        return Vector3.Distance(a.body.transform.position, b.body.transform.position) + 1000 * Mathf.Abs(a.body.transform.position.y - b.body.transform.position.y);
    }

    public static float Heuristic(PathNode p1, PathNode p2)
    {
        return Math.Abs(p1.body.transform.position.x - p2.body.transform.position.x) +
               Math.Abs(p1.body.transform.position.y - p2.body.transform.position.y) +
               Math.Abs(p1.body.transform.position.z - p2.body.transform.position.z);
    }
