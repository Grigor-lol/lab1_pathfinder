...
public class Grid : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int x = 0; x < sizeX; ++x)
            for (int z = 0; z < sizeZ; ++z)
            {
                ...
                grid[x, z] = new PathNode(new Vector2Int(x, z), nodeModel, false, position);
    ...
    void calculateDijkstra(Vector2Int start, Vector2Int finish)
    {
        //  Очищаем все узлы - сбрасываем отметку родителя, снимаем подсветку
        foreach (var node in grid)
        {
        ...
        while (nodes.Count != 0)
        {
            ...
            //  Если достали целевую - можно заканчивать (это верно и для A*)
            if (currentNode.gridVec == finish) break;
            //  Получаем список соседей
            var neighbours = GetNeighbours(currentNode.gridVec);
            foreach (var node in neighbours)
            {
                var neighbourNode = grid[node.x, node.y];
        ...
        //  Восстанавливаем путь от целевой к стартовой
        var pathElem = grid[finish.x, finish.y];
        while (pathElem != null)
        {
            pathElem.FadeToYellow();
            pathElem = pathElem.ParentNode;
    ...
    void calculateAStar(Vector2Int start, Vector2Int finish)
    {
        //  Очищаем все узлы - сбрасываем отметку родителя, снимаем подсветку
        foreach (var node in grid)
        {
        ...
        while (nodes.Count != 0)
        {
            ...
            {
                var neighbourNode = grid[node.x, node.y];
                if (neighbourNode.walkable && neighbourNode.Distance > currentNode.Distance + PathNode.Dist(neighbourNode, currentNode) + PathNode.Heuristic(neighbourNode, grid[finish.x, finish.y]))
                {
                    neighbourNode.ParentNode = currentNode;
                    neighbourNode.Distance += PathNode.Heuristic(neighbourNode, grid[finish.x, finish.y]);
                    nodes.Enqueue(neighbourNode);
                }
            }
        }
    ...
    void Update()
    {
        ...
        calculateAStar(new Vector2Int(0, 0), new Vector2Int(grid.GetLength(0) - 1, grid.GetLength(1) - 1));
    }
}